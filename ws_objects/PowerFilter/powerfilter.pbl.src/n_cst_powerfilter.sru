...
Integer	ButtonWidth = 69 //default button width
...
ELSE
	...
	if	Len(is_ColumnFilter[li_colnum]) > 0 then
		iu_powerfilter_dropdown.ii_buttonsrow3[li_colnum] = 1
	end if
...
IF this.checked THEN
	...
	is_previewbuttons = idw_dw.Describe("DataWindow.Print.Preview.Buttons") //save original setting
	IF upper(idw_dw.Describe("DataWindow.Print.Preview")) = 'YES' THEN
...
ls_modstring = &
"create button(band=Foreground text='' filename='PF_DownArrow_PF.png' suppresseventprocessing=yes enabled=yes action='0' border='0' color='33554432' "  +  &
"x='" +string(ll_xpos)+ "' "  +  &
"y='" +string(li_ypos)+ "' "  +  &
