...
forward
global type w_report_view from w_sheet
end type
...
type dw_report from u_dw within w_report_view
end type
type st_title from statictext within w_report_view
end type
type st_objects from structure within w_report_view
end type
end forward
...
private st_objects		i_st_object_list [ ]

Long                         i_l_rows_per_page

powerobject Requestor
...
public function integer setrequestor (ref powerobject setrequestor)
end prototypes
...
ELSEIF i_st_parm.value11 > '' THEN
	if isnull(i_st_parm.value12,'') = '' then 
		SELECT	type,
			object_name,
			library_name,
			preview,
...
public function integer setrequestor (ref powerobject setrequestor);
if	not IsValid(setRequestor) or IsNull(setRequestor) then return FAILURE
...
int iCurrent
call super::create
if this.MenuName = "m_report_view" then this.MenuID = create m_report_view
...
of_SetResize(true)
inv_Resize.of_SetOrigSize(dw_report.x + dw_report.width, uo_1.y + uo_1.height)
inv_Resize.of_Register(st_title, 0, 0, 100, 0)
inv_Resize.of_Register(dw_report, 0, 0, 100, 100)
inv_Resize.of_Register(ln_1, 0, 100, 100, 0)
inv_Resize.of_Register(ln_2, 0, 100, 100, 0)
inv_Resize.of_Register(uo_1, 0, 100, 0, 0)
