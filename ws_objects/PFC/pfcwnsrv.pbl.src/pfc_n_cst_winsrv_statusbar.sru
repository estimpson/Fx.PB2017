...
type os_dwobjects from structure
	string		s_id
	string		s_type
	string		s_value
	integer		i_width
...
ls_header = &
	...
	'datawindow(units=0 '+ &
		ls_dwtimerinterval + &
		'color=80263328 processing=0 print.documentname="" ' + &
...
For li_cnt = 1 to li_upper
	...
	If Len(Trim(istr_dwobjects[li_cnt].s_id)) = 0 Then
		Continue
	End If

	// Define the X point for the new object.
	li_xpoint = li_currentwidth + istr_dwobjects[li_cnt].i_gapwidth
	
	CHOOSE CASE Lower(istr_dwobjects[li_cnt].s_type)
		CASE TEXT
			ls_newobject = + &
				'compute( '+ &
					'name=' + istr_dwobjects[li_cnt].s_id + ' ' + &
					'expression="~'' + string(istr_dwobjects[li_cnt].s_value) + '~'" '+ &
					'border="' + string(istr_dwobjects[li_cnt].i_bordertype) +'" ' + &
					'x="' + string(li_xpoint) + '" ' + &
					'height="'+string(li_microhelpheight)+'" ' + &	
					'width="'+ string(istr_dwobjects[li_cnt].i_width) + '" ' + &
		...
		CASE BITMAP
			ls_newobject = + &
				'compute( '+ &
					'name=' + istr_dwobjects[li_cnt].s_id + ' ' + &
					'expression="bitmap(~'' + string(istr_dwobjects[li_cnt].s_value) + '~')" '+ &
					'border="' + string(istr_dwobjects[li_cnt].i_bordertype) +'" ' + &
					'x="' + string(li_xpoint) + '" ' + &
					'width="'+ string(istr_dwobjects[li_cnt].i_width) + '" ' + &
					'height="'+string(li_microhelpheight)+'" ' + &	
		...
		CASE PREDEFINED
			If istr_dwobjects[li_cnt].s_id = 'pfc_timer' Then
				ls_newobject = + &			
					'compute( '+ &
						'name=' + istr_dwobjects[li_cnt].s_id + ' ' + &
						'expression="Today()" '+ &					
						...
						'font.family="2" font.pitch="2" font.charset="0" background.mode="2" ' +&
					') '			
			ElseIf istr_dwobjects[li_cnt].s_id = 'pfc_progress' Then
				ls_newobject = + &			
					'rectangle( '+ &
						'name=' + istr_dwobjects[li_cnt].s_id + ' ' + &
						'x="' + string(li_xpoint + ii_baroffsetx) + '" ' + &
						'width="'+ string(istr_dwobjects[li_cnt].i_width - ii_baroffsetx) + '" ' + &
					...
					') ' + &			
					'compute( '+ &
						'name=' + istr_dwobjects[li_cnt].s_id + '_border ' + &
						'expression="" '+ &					
						'border="' + string(istr_dwobjects[li_cnt].i_bordertype) +'" ' + &
						'x="' + string(li_xpoint) + '" ' + &
						'width="'+ string(istr_dwobjects[li_cnt].i_width) + '" ' + &
					...
					') '		
					ii_barwidth  = istr_dwobjects[li_cnt].i_width - ii_baroffsetx
	...
	li_currentwidth = li_xpoint + istr_dwobjects[li_cnt].i_width
Next 
...
// Add the new object.
istr_dwobjects[li_upper].s_id = as_id
istr_dwobjects[li_upper].s_type = 'predefined'
istr_dwobjects[li_upper].i_width = ai_width
istr_dwobjects[li_upper].i_bordertype = ii_bordertype
istr_dwobjects[li_upper].i_gapwidth = ii_gapwidth
...
For li_cnt = 1 to li_upper
	If as_id = istr_dwobjects[li_cnt].s_id Then
		istr_dwobjects[li_cnt].s_id = ''
		istr_dwobjects[li_cnt].s_type = ''
		istr_dwobjects[li_cnt].s_value = ''
		istr_dwobjects[li_cnt].i_width = 0
...
// Add the new object.
istr_dwobjects[li_upper].s_id = as_id
istr_dwobjects[li_upper].s_type = as_type
istr_dwobjects[li_upper].s_value = as_value
istr_dwobjects[li_upper].i_width = ai_width
istr_dwobjects[li_upper].i_bordertype = ai_bordertype
istr_dwobjects[li_upper].i_gapwidth = ai_gapwidth
...
// Update the object.
istr_dwobjects[li_id].i_width = ai_width
