...
protected function integer savegriddw (boolean ab_useregistry, string as_keyorini, string as_inisection, windowobject controls[])
protected function integer restoregriddw (boolean ab_useregistry, string as_keyorini, string as_inisection, windowobject controls[])
public function boolean hasfxdwancestry (datawindow testdw)
protected function integer savedefaultgridfilter (boolean useregistry, string keyorini, datawindow dwcontrol, string inisection)
protected function integer savefxgridcolumnfilters (boolean useregistry, string keyorini, n_cst_dwsrv_filter filterservice, string inisection)
protected function integer savepowerfilter (boolean useregistry, string keyorini, n_cst_powerfilter filterservice, string inisection)
protected function integer restorepowerfilter (boolean useregistry, string keyorini, n_cst_powerfilter filterservice, string inisection)
protected function integer restoresplitters (boolean ab_useregistry, string as_keyorini, string as_inisection, windowobject controls[])
protected function integer savesplitters (boolean ab_useregistry, string as_keyorini, string as_inisection, windowobject controls[])
end prototypes
...
//	Check for magic key to skip processing.
...
//	Save splitters.
SaveSplitters(ab_UseRegistry, as_KeyOrIni, as_IniSection, iw_requestor.control)

//	Save grid dataindows.
SaveGridDW(ab_UseRegistry, as_KeyOrIni, as_IniSection, iw_requestor.control)
...
If ib_window Then
	...
	//	Record this information as the "normal state".
	...
	//	Restore windowstate normal or maximized.
	li_rc = of_Restore (ab_UseRegistry, as_KeyOrIni, as_IniSection, &
			'window.windowstate', ls_windowstate, '')
	if lnv_conversion.of_WindowState(ls_windowstate, lws_windowstate) = 1 then
		iw_requestor.windowstate = lws_WindowState
	end if
End If

RestoreSplitters(ab_UseRegistry, as_KeyOrIni, as_IniSection, iw_Requestor.control)

if	restoreGridDW then
	li_rc = RestoreGridDW(ab_UseRegistry, as_KeyOrIni, as_IniSection, iw_Requestor.control)
end if
...
protected function integer savegriddw (boolean ab_useregistry, string as_keyorini, string as_inisection, windowobject controls[]);
//	Loop through all controls on Window and restore column properties of Grid DWs.
int i
int rc
...
for i = 1 to upperbound(controls)
	...
	choose case control.TypeOf()
		case DataWindow!
			//	Do something.
			datawindow dwControl
			dwControl = control
			if	dwControl.dataobject = "" then continue
			string checkSyntax
			checkSyntax = dwControl.Describe("DataWindow.Syntax")
			if	len(checkSyntax) <= 1 then continue
			if	dwControl.object.DataWindow.Processing = "1" then
				
				//	Record column widths.
				string	objectList[]
				objectList = SplitString(dwControl.Describe("DataWindow.Objects"), "~t")
				int j
				for j = 1 to UpperBound(objectList)
					rc = of_Save (ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), as_IniSection, objectList[j] + ".width", dwControl.Describe(objectList[j] + ".width"))
				next
				
				//	Record datawindow filter.
				if	HasFXDWAncestry(dwControl) then
					//	Check to see if fxdw is using filter service.
					u_fxdw fxdwControl
					fxdwControl = dwControl
					if	IsValid(fxdwControl.PowerFilter) then
						rc = savePowerFilter(ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), fxdwControl.PowerFilter, as_IniSection)
					elseif IsValid(fxdwControl.inv_Filter) then
						//	Save column filters.
						rc = saveFXGridColumnFilters(ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), fxdwControl.inv_Filter, as_IniSection)
					else
						//	Perform default save of grid filter.
						rc = saveDefaultGridFilter(ab_UseRegistry, as_KeyOrIni, dwControl, as_IniSection)
					end if						
				else
					//	Perform default save of grid filter.
					rc = saveDefaultGridFilter(ab_UseRegistry, as_KeyOrIni, dwControl, as_IniSection)
				...
				//	Record datawindow sort.
				string dwSort
				dwSort = dwControl.Describe("DataWindow.Table.Sort")
				rc = of_Save (ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), as_IniSection, "table.sort", dwSort)
			end if
		case UserObject!
			//	Get nesting...
			userobject uoChildControl
			uoChildControl = control
			if	UpperBound(uoChildControl.control) > 0 then
				SaveGridDW (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, uoChildControl.control)
			end if
		case Tab!
			//	Get nesting...
			tab tabControl
			tabControl = control
			SaveGridDW (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, tabControl.control)
	...
	choose case control.TypeOf()
		case DataWindow!
			...
			if	dwControl.object.DataWindow.Processing = "1" then
				...
				int j
				for j = 1 to UpperBound(objectList)
					rc = of_Restore (ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), as_IniSection, objectList[j] + ".width", width, "")
					if	width <> "?" and width <> "" then
						modifySyntax += objectList[j] + ".width=" + width + " "
					end if
				next
				dwControl.Modify(modifySyntax)
				
				//	Restore datawindow filter.
				string dwFilter
				if	HasFXDWAncestry(dwControl) then
					//	Check to see if fxdw is using filter service.
					u_fxdw fxdwControl
					fxdwControl = dwControl
					if	IsValid(fxdwControl.PowerFilter) then
						rc = RestorePowerFilter(ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), fxdwControl.PowerFilter, as_IniSection)
					else
						//	Perform default save of grid filter.
						rc = of_Restore (ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), as_IniSection, "table.filter", dwFilter, "")
						if	dwFilter <> "?" and dwFilter <> "" and dwFilter<> "!" then
							//dwControl.SetFilter(dwFilter)
							dwControl.object.DataWindow.Table.Filter = dwFilter
							dwControl.Filter()
						end if
					end if						
				else
				...
				string dwSort
				rc = of_Restore (ab_UseRegistry, as_KeyOrIni + "\" + dwControl.ClassName(), as_IniSection, "table.sort", dwSort, "")
				if	dwSort <> "?" and dwSort <> ""  and dwSort<> "!" then
					dwControl.SetSort(dwSort)
					dwControl.Sort()
				end if
			end if
		case UserObject!
			...
			userobject uoChildControl
			uoChildControl = control
			RestoreGridDW (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, uoChildControl.control)
		case Tab!
			//	Get nesting...
			tab tabControl
			tabControl = control
			RestoreGridDW (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, tabControl.control)
...
public function boolean hasfxdwancestry (datawindow testdw);
classdefinition testDWClassDefinition, parentDef

//	Ensure specified class is derived from a w_sheet_4t window.
boolean IsFXDW = false
parentDef = testDW.ClassDefinition
do while IsValid(parentDef) and not IsFXDW
	if	parentDef.Name = "u_fxdw" then
		IsFXDW = true
		continue
	end if
...
int returnValue
string dwFilter
dwFilter = dwControl.object.DataWindow.Table.Filter
returnValue = of_Save (useRegistry, keyOrIni + "\" + dwControl.ClassName(), iniSection, "table.filter", dwFilter)
...
//	Default filter handling.
int returnValue
string dwFilter, columnName
//dwFilter = dwControl.object.DataWindow.Table.Filter
returnValue = of_Save (useRegistry, keyOrIni + "\" + columnName, iniSection, "table.filter", dwFilter)
...
protected function integer savepowerfilter (boolean useregistry, string keyorini, n_cst_powerfilter filterservice, string inisection);
//	PowerFilter handling.
int returnValue

string columnFilters[]
int i, columnNumbers[]

filterService.GetFilters(columnFilters, columnNumbers)
for	i = 1 to UpperBound(columnFilters)
	returnValue = of_Save (useRegistry, keyOrIni, iniSection, "table.PowerFilter." + String(i), columnFilters[i])
	returnValue = of_Save (useRegistry, keyOrIni, iniSection, "table.PowerFilter.column#." + String(i), String(columnNumbers[i]))
next
returnValue = of_Save (useRegistry, keyOrIni, iniSection, "table.PowerFilter.columns", String(UpperBound(columnFilters)))
...
string filterColumns, columnFilter, columnNumber, columnFilters[]
int i, columnNumbers[]

returnValue = of_Restore (useRegistry, keyOrIni, iniSection, "table.PowerFilter.columns", filterColumns, "0")
for	i = 1 to Long(filterColumns)
	returnValue = of_Restore (useRegistry, keyOrIni, iniSection, "table.PowerFilter." + string(i), columnFilter, "0")
	returnValue = of_Restore (useRegistry, keyOrIni, iniSection, "table.PowerFilter.column#." + string(i), columnNumber, "0")
	columnFilters[i] = columnFilter
	columnNumbers[i] = Long(columnNumber)
next
filterService.post BuildFilters(columnFilters, columnNumbers)
...
for i = 1 to upperbound(controls)
	...
	choose case control.TypeOf()
		case StaticText!
			//	Do something.
			statictext stControl
			stControl = control
			ClassDefinition parentDef
			parentDef = stControl.ClassDefinition
			boolean splitBar
			splitBar = false
			do while IsValid(parentDef) and not splitBar
				if	parentDef.Name = "u_st_splitbar" then
					splitBar = true
					exit
				end if
				parentDef = parentDef.Ancestor
			loop
			if	splitBar then
				ClassDefinition splitterDef
				splitterDef = stControl.ClassDefinition
				string x, y
				rc = of_Restore (ab_UseRegistry, as_KeyOrIni + "\" + stControl.ClassName(), as_IniSection, "x", x, "")
				rc = of_Restore (ab_UseRegistry, as_KeyOrIni + "\" + stControl.ClassName(), as_IniSection, "y", y, "")
				if	x <> "?" and x <> "" and y <> "?" and y <> "" then
					u_st_splitbar splitBarControl
					splitBarControl = stControl
					if	isnull(splitBarControl.Tag, '') <> 'I' then
						splitBarControl.post SetSplitbarPosition (Long(X), Long(Y))
					end if
				end if
			end if
		case UserObject!
			...
			userobject uoChildControl
			uoChildControl = control
			RestoreSplitters (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, uoChildControl.control)
		case Tab!
			//	Get nesting...
			tab tabControl
			tabControl = control
			RestoreSplitters (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, tabControl.control)
	end choose
next
	...
	choose case control.TypeOf()
		case StaticText!
			...
			if	splitBar then
				ClassDefinition splitterDef
				splitterDef = stControl.ClassDefinition
				rc = of_Save (ab_UseRegistry, as_KeyOrIni + "\" + stControl.ClassName(), as_IniSection, "x", String(stControl.x))
				rc = of_Save (ab_UseRegistry, as_KeyOrIni + "\" + stControl.ClassName(), as_IniSection, "y", String(stControl.y))
			end if
		case UserObject!
			//	Get nesting...
			userobject uoChildControl
			uoChildControl = control
			if	UpperBound(uoChildControl.control) > 0 then
				SaveSplitters (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, uoChildControl.control)
			end if
		case Tab!
			//	Get nesting...
			tab tabControl
			tabControl = control
			SaveSplitters (ab_UseRegistry, as_KeyOrIni + "\" + control.ClassName(), as_Inisection, tabControl.control)
	end choose
next
