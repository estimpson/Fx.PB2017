...
If ienv_object.Win16 Then
	// win.ini.
	ls_buttonface = ProfileString("win.ini", "colors", "buttonface", "")
	
	li_border = 	integer(ProfileString("win.ini", "windows", "borderwidth", "2")) 
	li_border *= 15
	ii_borderheight = unitstopixels(li_border, yunitstopixels!) - 16
Else
	If ienv_object.OSType = WindowsNT! And ienv_object.OSMajorRevision>= 4 Then
		// NT 4.0 registry.
		registryget('hkey_current_user\control panel\colors', 'buttonface' ,ls_buttonface)
	
		registryget('hkey_current_user\control panel\desktop\windowmetrics','borderwidth',ls_temp)
		li_border = abs(integer(ls_temp)) * 15
		ii_borderheight = unitstopixels(li_border, yunitstopixels!)
	ElseIf ienv_object.OSType = WindowsNT! And ienv_object.OSMajorRevision< 4 Then
		// Pre NT 4.0 registry.
		registryget('hkey_current_user\control panel\colors', 'buttonface' ,ls_buttonface)
	
		registryget('hkey_current_user\control panel\desktop','borderwidth',ls_temp)
		li_border = abs(integer(ls_temp)) * 15
		ii_borderheight = unitstopixels(li_border, yunitstopixels!) - 16
	Else
		// win95 registry.
