$PBExportHeader$n_cst_dwsrv_search.sru
$PBExportComments$Extension DataWindow Search service
forward
global type n_cst_dwsrv_search from n_cst_dwsrv
end type
type os_columns from structure within n_cst_dwsrv_search
end type
end forward
...
global n_cst_dwsrv_search n_cst_dwsrv_search
...
public function integer of_register (string as_column, dwobject adwo_source, datawindow adw_source)
end prototypes
...
//	Search the source.
long	ll_RowCount
string	ls_SearchColumn
ls_SearchColumn = istr_Columns [ii_CurrentIndex].dwo_source.Name
ls_FindExp =  "Lower (Left (" + ls_SearchColumn + ", " + String (li_searchtextlen) + ")) = '" + Lower (ls_searchtext) + "'"
ll_RowCount = istr_Columns [ii_CurrentIndex].dw_source.RowCount ()
ll_FindRow = istr_Columns [ii_CurrentIndex].dw_source.Find (ls_FindExp, 0, ll_RowCount)

//	Determine if a match was found.
lb_MatchFound = (ll_FindRow > 0)

//	Set the found text and scroll to row.
if lb_MatchFound then
	ls_FoundText = istr_Columns [ii_CurrentIndex].dw_source.GetItemString (ll_FindRow, ls_SearchColumn)
	istr_Columns [ii_CurrentIndex].dw_source.ScrollToRow (ll_FindRow)
	istr_Columns [ii_CurrentIndex].dw_source.SetRow (ll_FindRow)
end if
...
If IsNull(as_column) Or Len(Trim(as_column))=0 Or &
	IsNull(idw_requestor) Or Not IsValid(idw_requestor) or not IsValid (adwo_Source) or not IsValid (adw_Source) Then Return -1

// Add the new entry.
li_count = upperbound(istr_columns) +1					
istr_columns[li_count].s_columnname = as_column
istr_columns[li_count].dwo_source = adwo_Source
istr_columns[li_count].dw_source = adw_Source
Return 1
...
on n_cst_dwsrv_search.destroy
call super::destroy
end on

on n_cst_dwsrv_search.create
call super::create
end on
