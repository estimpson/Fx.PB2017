...
public function integer setsplitbarposition (integer ai_xpos, integer ai_ypos)
end prototypes
...
//	Notify frame that Split Bar is no longer in use.
gnv_App.of_GetFrame().event pfc_splitbarend()

//	Ensure splitter on top.
...
//	Notify frame that Split Bar is in use.
if	gnv_App.of_GetFrame().event pfc_splitbarbegin() = w_frame.PREVENT_ACTION then
	return u_base.FAILURE
end if
...
public function integer setsplitbarposition (integer ai_xpos, integer ai_ypos);Integer	li_cnt
Integer 	li_deltax, li_deltay
Integer	li_pointerx, li_pointery
Integer	li_minx, li_maxx, li_miny, li_maxy
Integer	li_x, li_y, li_width, li_height
