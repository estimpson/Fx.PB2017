...
type os_updcharacteristics from structure
...
type variables
Protected:
os_updcharacteristics istr_newupdate[]
end variables
...
protected function integer of_restoreupdatesettings (os_updcharacteristics astr_originalupdate)
protected function integer of_storeupdatesettings (ref os_updcharacteristics astr_originalupdate)
...
os_updcharacteristics	lstr_originalupdate
...
FOR li_tabidx = 1 to li_num_tables
	...
	ls_mod_string = "DataWindow.Table.UpdateTable='" + &
						 istr_newupdate[li_tabidx].s_updatetable +"' "
	
	// Set the Update Where Option.
	ls_mod_string += "DataWindow.Table.UpdateWhere=" + &
						 String(istr_newupdate[li_tabidx].i_whereoption) +" " 
	
	// Set the Update Key in Place flag.
	If istr_newupdate[li_tabidx].b_keyupdateinplace THEN
	...
	li_num_updateable = UpperBound ( istr_newupdate[li_tabidx].s_updatecolumns )
	li_num_keys = UpperBound ( istr_newupdate[li_tabidx].s_keycolumns )
	...
	FOR li_updcolidx = 1 to li_num_cols
		...
		FOR li_i = 1 to li_num_updateable
			IF Lower(ls_column) = Lower(istr_newupdate[li_tabidx].s_updatecolumns[li_i]) THEN 
		...
		FOR li_keycolidx = 1 to li_num_keys
			IF Lower(ls_column) = Lower(istr_newupdate[li_tabidx].s_keycolumns[li_keycolidx]) THEN 
				lb_is_key = TRUE
				EXIT
			END IF 
		NEXT 
...
li_rc = of_RestoreUpdateSettings ( lstr_originalupdate ) 
If li_rc < 0 Then Return li_rc
...
public function integer of_addtoupdate (string as_table, string as_keycolumns[], string as_updateable_cols[]);//////////////////////////////////////////////////////////////////////////////
//
//	Function:  of_AddToUpdate ( Format 2 ) 
//
//	Access:    Public
//
//	Arguments: 
...
//
//	Arguments: 		
//	astr_originalupdate 	Original Update information.
//
//	Returns:   		Integer
...
ls_mod_string = "DataWindow.Table.UpdateTable='" + astr_originalupdate.s_updatetable + "' "

// Restore the Update Where statement.
ls_mod_string = ls_mod_string + &
					 "DataWindow.Table.UpdateWhere=" + String(astr_originalupdate.i_whereoption) + " "

// Restore the Update Key In Place flag.
If astr_originalupdate.b_keyupdateinplace Then
...
li_num_updateable = UpperBound (astr_originalupdate.s_updatecolumns) 
li_num_keys = UpperBound (astr_originalupdate.s_keycolumns) 
...
FOR li_colidx = 1 to li_num_cols
	...
	FOR li_updcolidx = 1 to li_num_updateable 
		IF astr_originalupdate.s_updatecolumns [li_updcolidx] = ls_column THEN 
			lb_found = TRUE
			Exit
		END IF 
	NEXT
	...
	lb_found = FALSE	
	FOR li_keycolidx = 1 to li_num_keys
		IF astr_originalupdate.s_keycolumns [li_keycolidx] = ls_column THEN 
			lb_found = TRUE
			Exit
		END IF 
	NEXT
...
//
//	Arguments: 		
//	astr_originalupdate 	Original Update information (passed by reference).
//
//	Returns:   		Integer
...
IF Lower(idw_Requestor.Describe("DataWindow.Table.UpdateKeyInPlace")) = "yes" THEN 
   astr_originalupdate.b_keyupdateinplace = TRUE
ELSE
   astr_originalupdate.b_keyupdateinplace = FALSE
END IF 
...
FOR li_colidx = 1 to li_numcols
	...
	//If a column was set as updateable, then store it in the structure.
	IF Lower(idw_Requestor.Describe (ls_column + ".Update" )) = "yes" THEN
		astr_originalupdate.s_updatecolumns[UpperBound(astr_originalupdate.s_updatecolumns)+1] = ls_column
	END IF 
	
	//If a column was set as a key column, then store it in the structure.
	IF Lower(idw_Requestor.Describe (ls_column + ".Key" )) = "yes" THEN
		astr_originalupdate.s_keycolumns[UpperBound(astr_originalupdate.s_keycolumns)+1] = ls_column
	END IF 
...
li_registeredcount = UpperBound ( istr_newupdate ) 
For li_cnt = 1 to li_registeredcount
	If istr_newupdate[li_cnt].s_updatetable = Trim(as_table) Then
...
li_tablecount = UpperBound ( istr_newupdate ) 
For li_cnt = 1 to li_tablecount
	If Trim(istr_newupdate[li_cnt].s_updatetable) = '' Then
		li_openslot = li_cnt
		Exit
	End If
Next
...
// Add this table to the structure of update tables 
istr_newupdate[li_openslot].s_updatetable = Trim(as_table)
istr_newupdate[li_openslot].i_whereoption = ai_whereoption
istr_newupdate[li_openslot].b_keyupdateinplace = ab_keyinplace
istr_newupdate[li_openslot].s_updatecolumns = as_updateablecolumns
istr_newupdate[li_openslot].s_keycolumns = as_keycolumns
...
li_registeredcount = UpperBound ( istr_newupdate ) 
For li_cnt = 1 to li_registeredcount
	If Trim(istr_newupdate[li_cnt].s_updatetable) = Trim(as_table) Then
		// The table to be unregisterd has been found. 
		istr_newupdate[li_cnt].s_updatetable = ''
		istr_newupdate[li_cnt].i_whereoption = 0
		istr_newupdate[li_cnt].b_keyupdateinplace = False
		istr_newupdate[li_cnt].s_updatecolumns = ls_empty
		istr_newupdate[li_cnt].s_keycolumns = ls_empty
		Return 1
	End If
Next
...
li_registeredcount = UpperBound ( istr_newupdate ) 
For li_cnt = 1 to li_registeredcount
	If Len(Trim(istr_newupdate[li_cnt].s_updatetable)) > 0 Then
...
For li_cnt = 1 to li_tablecount
	If Trim(istr_newupdate[li_cnt].s_updatetable) = Trim(as_table) Then
		...
		as_keycolumns = istr_newupdate[li_cnt].s_keycolumns
		as_updateablecolumns = istr_newupdate[li_cnt].s_updatecolumns
		ab_keyinplace = istr_newupdate[li_cnt].b_keyupdateinplace
		ai_whereoption = istr_newupdate[li_cnt].i_whereoption
		Return 1
	End If
Next
