...
n_cst_dwsrv_querymodeinfo istr_QueryModeInfo[]
...
public function integer setquerycols ()
end prototypes
...
if li_maxcols = 0 then
	...
	// as being a non-queryable colum.
	...
	for li_i = 1 to li_maxcols
		istr_querymodeinfo[li_i].ColumnName = ls_collist[li_i]
		istr_querymodeinfo[li_i].State = false
		istr_querymodeinfo[li_i].ProtectExpression = idw_requestor.Describe (ls_collist[li_i] + ".Protect")
		istr_querymodeinfo[li_i].TabSequence = long(idw_requestor.Describe (ls_collist[li_i] + ".TabSequence"))
	next
...
for li_i = 1 to li_maxset
	for li_j = 1 to li_maxcols
		if Lower (istr_querymodeinfo[li_j].ColumnName) = Lower (as_querycolumns[li_i]) then
			istr_querymodeinfo[li_j].State = true
		end if
	next
next
...
choose case ab_switch
		...
		case true
			...
			If lb_querycolumn Then
				...
				for li_i = 1 to li_numquerycols
					if istr_querymodeinfo[li_i].State = false then
						// Add to modify string to protect for non-querymode columns
						ls_modify = ls_modify + istr_querymodeinfo[li_i].ColumnName + ".Protect = 1 " 
					else
						// Add to modify string to non-protect for querymode columns.
						ls_modify = ls_modify + istr_querymodeinfo[li_i].ColumnName + ".Protect = 0 " 
						ls_modify = ls_modify + istr_querymodeinfo[li_i].ColumnName + ".TabSequence = " + String(li_i) + " "
					end if
				next
		...
		case false
			...
			for li_i = 1 to li_numquerycols
				...
				ls_modify = ls_modify + istr_querymodeinfo[li_i].ColumnName + &
						".Protect = " + istr_querymodeinfo[li_i].ProtectExpression + " " 

				ls_modify = ls_modify + istr_querymodeinfo[li_i].ColumnName + &
						".TabSequence = " + String(istr_querymodeinfo[li_i].TabSequence) + " " 
			next
...
public function integer setquerycols ();string	columnList[]
of_GetObjects (columnList, "column", "*", true) 
return of_SetQueryCols(columnList)
