...
type os_columns from structure within pfc_n_cst_dwsrv_dropdownsearch
end type
end forward
...
If istr_columns[ii_currentindex].s_editstyle = 'dddw' Then
	...
	idw_requestor.GetChild(ls_searchcolname, istr_columns[ii_currentindex].dwc_object)
...
If ls_editstyle = 'dddw' Then
	If IsNull(istr_columns[li_index].dwc_object) Or 	Not isValid(istr_columns[li_index].dwc_object) Then
		adwo_object.getchild(ls_dwcolname, istr_columns[li_index].dwc_object)
		datawindowchild dwc_object
		int i
		i = adwo_object.getchild(ls_dwcolname, dwc_object)
		messagebox('', String(i))
	End If
End If
...
// Check that dataobject has been set.
If idw_requestor.DataObject = "" Then
	Return -1
End If

if	not isValid(idw_requestor.object) then
	return -1
end if
