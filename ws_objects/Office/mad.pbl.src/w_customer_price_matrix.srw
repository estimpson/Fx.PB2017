...
forward
global type w_customer_price_matrix from w_sheet_4t
end type
...
OpenSheetWithParm ( w_part_package_materials, szPart, gnv_App.of_GetFrame(), 0, Original! )
end event
...
event open;call super::open;//  Set Hourglass
...
this.Control[iCurrent+1]=this.dw_add_profile
this.Control[iCurrent+2]=this.cb_add_cancel
this.Control[iCurrent+3]=this.cb_add_ok
this.Control[iCurrent+4]=this.st_add_title
this.Control[iCurrent+5]=this.st_2
this.Control[iCurrent+6]=this.st_1
this.Control[iCurrent+7]=this.dw_part_info
this.Control[iCurrent+8]=this.cb_add_back
this.Control[iCurrent+9]=this.dw_price_matrix
this.Control[iCurrent+10]=this.lb_temp
this.Control[iCurrent+11]=this.dw_customer_part_list
end on
		...
		IF SQLCA.SQLCODE = 0 THEN
				...
				IF lst_values.value1 = 'Y' THEN
						...
						IF SQLCA.SQLCode = 0 THEN
							...
							DECLARE shipperupdate2 PROCEDURE FOR 'cdisp_shipperpriceupdate'
								:ls_part, :ls_customer, :ld_price, 2 ;
							Execute shipperupdate2 ;
							IF sqlca.sqlcode < 0 THEN
				...
				IF lst_values.value2 = 'Y' THEN
					...
					DECLARE shipperupdate PROCEDURE FOR 'cdisp_shipperpriceupdate'
						:ls_part, :ls_customer, :ld_price, 0 ;
				...
				IF lst_values.value3 = 'Y' THEN
					...
					DECLARE shipperupdate1 PROCEDURE FOR 'cdisp_shipperpriceupdate'
						:ls_part, :ls_customer, :ld_price, 1 ;
...
//	Initialize Variables
l_Row = dw_customer_part_list.GetClickedRow ( )
