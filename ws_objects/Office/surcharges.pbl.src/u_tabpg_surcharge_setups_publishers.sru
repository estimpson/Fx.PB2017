$PBExportHeader$u_tabpg_surcharge_setups_publishers.sru
forward
global type u_tabpg_surcharge_setups_publishers from u_tabpg_dw
end type
type cb_1 from commandbutton within u_tabpg_surcharge_setups_publishers
end type
type cb_2 from commandbutton within u_tabpg_surcharge_setups_publishers
end type
type dw_quickgridmenu from u_fxdw_quickgridmenu within u_tabpg_surcharge_setups_publishers
end type
end forward
...
global u_tabpg_surcharge_setups_publishers u_tabpg_surcharge_setups_publishers
...
public function integer setpublisherlist ()
end prototypes
...
for i = 1 to UpperBound(selectedRows)
	if	selectedRows[i] = 1 then
		string publisher
		publisher = dw_1.Object.Publisher[i]
		publisherArray[UpperBound(publisherArray)+1] = publisher
	end if
next

if	UpperBound(publisherArray) > 0 then
	return _myController.SetPublisherList(publisherArray)
end if

return NO_ACTION
end function

on u_tabpg_surcharge_setups_publishers.create
int iCurrent
call super::create
...
on u_tabpg_surcharge_setups_publishers.destroy
call super::destroy
destroy(this.cb_1)
destroy(this.cb_2)
...
type gb_1 from u_tabpg_dw`gb_1 within u_tabpg_surcharge_setups_publishers
string text = "Publishers"
end type

type dw_1 from u_tabpg_dw`dw_1 within u_tabpg_surcharge_setups_publishers
string dataobject = "d_surcharge_publishers"
end type
