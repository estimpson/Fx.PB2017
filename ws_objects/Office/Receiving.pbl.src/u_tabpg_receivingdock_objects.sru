$PBExportHeader$u_tabpg_receivingdock_objects.sru
forward
...
public function integer setselectedrowscolumnvalue (string columnname, string newvalue)
...
for	row = 1 to rows
	if	SelectedArray[row] = 1 then
		dw_1.SetItem(row, columnName, newValue)
	end if
next
...
public function integer reset ();dw_1.Reset()
...
choose case lower(dwo.name)
	case "lot", "packagetype", "location", "plant", "note"
		SetSelectedRowsColumnValue(lower(dwo.name), data)
end choose
