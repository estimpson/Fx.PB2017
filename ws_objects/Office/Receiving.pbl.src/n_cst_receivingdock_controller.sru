$PBExportHeader$n_cst_receivingdock_controller.sru
forward
...
//	View data.
boolean _showInstructionsVisible = false
...
u_panel_headerdetail _receiverInstructionsPanel
u_fxdw_receivingdock_receiverheader _receiverInstructionsHeaderView
u_tabpg_receivingdock_instructions _receiverInstructionsView
...
public function integer toggleshowinstructions ()
public function integer showcontrols ()
public function integer getactivereceiverid (long receiverid)
end prototypes
...
//	Prepare instructions panel.
_parentSheet.uo_1.AddPanelControlGroup("ReceiverInstructions", n_panelcontrolgroup.VERTICAL)
_parentSheet.OpenUserObject(_receiverInstructionsHeaderView, "u_fxdw_receivingdock_receiverheader")
_parentSheet.uo_1.AddPanelControlToGroup("ReceiverInstructions", _receiverInstructionsHeaderView, _receiverInstructionsHeaderView.Width, _receiverInstructionsHeaderView.Height)

//	Receiving Instructions Preview:
_parentSheet.OpenUserObject(_receiverInstructionsPanel, "u_panel_headerdetail")
_parentSheet.uo_1.AddPanelControlToGroup("ReceiverInstructions", _receiverInstructionsPanel, _receiverInstructionsPanel.Width, -1)

_parentSheet.OpenUserObject(_receiverInstructionsView, "u_tabpg_receivingdock_instructions")
_receiverInstructionsPanel.uo_1.AddPanelControl(_receiverInstructionsView)
...
if	IsNull(_receiverNumber, "") <> IsNull(receiverNumber, "") then
	...
	_receiverObjectsView.Reset()
else
	if	_displayMode = MODE_SHOWLINES then
		_headerDetailPanel.uo_2.ShowControl(_receiverLinesView)
		_receiverLinesView.SetReceiverNumber(_receiverNumber)
...
public function integer toggleshowinstructions ();
_showInstructionsVisible = not _showInstructionsVisible
return ShowControls()
...
public function integer showcontrols ();
if	_showInstructionsVisible then
	//	Show Instructions:
	_receiverInstructionsHeaderView.SetReceiverNumber(_receiverNumber)
	_receiverInstructionsView.SetReceiverNumber(_receiverNumber)
	_parentSheet.uo_1.ShowControlGroup("ReceiverInstructions")
	_receiverInstructionsPanel.uo_1.ShowControl(_receiverInstructionsView)
	
else
	//	Display receiver header detail.
...
public function integer getactivereceiverid (long receiverid);
return _receivingDockTrans.GetReceiverID_FromReceiverNumber(_receiverNumber, receiverID)
