$PBExportHeader$n_cst_receivingdocktrans.sru
forward
global type n_cst_receivingdocktrans from n_cst_fxtrans
end type
end forward

global type n_cst_receivingdocktrans from n_cst_fxtrans
end type
global n_cst_receivingdocktrans n_cst_receivingdocktrans
...
public function integer putawayserial (string receivernumber, long putawayserial, string putawaylocation)
public function integer getreceiverobjectserial (long receiverobjectid, ref long serialnumber)
public function integer getreceiverid_fromreceivernumber (string receivernumber, ref long receiverid)
end prototypes
...
//	Attempt to create delete normal order.
declare PutAwaySerial procedure for dbo.usp_ReceivingDock_PutAwaySerial
	@User = :User
,	@ReceiverNumber = :receiverNumber
,	@PutAwaySerial = :putAwaySerial
,	@PutAwayLocation = :putAwayLocation
,	@TranDT = :tranDT output
,	@Result =:procResult output using TransObject  ;

execute PutAwaySerial  ;
sqlResult = SQLCA.SQLCode

if	sqlResult <> 0 then
	sqlError = SQLCA.SQLErrText
	SQLCA.of_Rollback()
	MessageBox(monsys.msg_Title, "Failed on execute, unable to put away object " + String(putAwaySerial) + " to location " +  putAwayLocation + ":  {" + String(sqlResult) + "," + String(procResult) + "} " + sqlError)
	return FAILURE
end if
...
if	procResult <> 0 or SQLCA.SQLCode <> 0 then
	sqlError = SQLCA.SQLErrText
	SQLCA.of_Rollback()
	MessageBox(monsys.msg_Title, "Failed on result, unable to put away object " + String(putAwaySerial) + " to location " +  putAwayLocation + ":  {" + String(sqlResult) + "," + String(procResult) + "} " + sqlError)
	return FAILURE
end if
...
on n_cst_receivingdocktrans.create
call super::create
end on

on n_cst_receivingdocktrans.destroy
call super::destroy
end on
