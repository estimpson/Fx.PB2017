...
If LeftA ( szReturnedString, 1 ) = "W" Then
	idec_ScaleWeight = Dec ( RightA ( szReturnedString, LenA ( szReturnedString ) - 1 ) )
	sle_qty.Text = "1"
Elseif LeftA ( szReturnedString, 1 ) = "A" Then
	idec_ScaleWeight = Dec ( RightA ( szReturnedString, LenA ( szReturnedString ) - 1 ) )
	sle_qty.Text = "1"
	ib_Auto = TRUE
Elseif LeftA ( szReturnedString, 1 ) = "N" Then
	idec_ScaleWeight = Dec ( RightA ( szReturnedString, LenA ( szReturnedString ) - 1 ) )
	sle_qty.Text = "1"
	ib_Auto = TRUE
...
Elseif bSerialNumber Then
	...
	If Not bBack Then
		...
		If Upper ( LeftA ( sle_serial.Text, 1 ) ) = 'S' Then
			sle_serial.Text = RightA ( sle_serial.Text, LenA ( sle_serial.Text ) - 1 )
		Elseif Not IsNumber ( LeftA ( sle_serial.Text, 1 ) ) Then
			f_beep ( 5 )
			sle_serial.Text = ""
...
Elseif bMachines Or bParts Or bLocationTo Or bPackaging Then
	...
	If ( bMachines And bPopulateMachines ) Or ( bParts And bPopulateParts ) &
		...
		Elseif KeyDown ( keyBack! ) Then
			...
			ddlbCurrentObject.Text = LeftA ( ddlbCurrentObject.Text, LenA ( ddlbCurrentObject.Text ) - 1 )
			szRealityText = LeftA ( szRealityText, LenA ( szRealityText ) - 1 )
			cCharacterTouched = ""
			Timer ( .055, This )

		Elseif KeyDown ( keyDelete! ) Then

			ddlbCurrentObject.Text = RightA ( ddlbCurrentObject.Text, LenA ( ddlbCurrentObject.Text ) - 1 )
			szRealityText = RightA ( szRealityText, LenA ( szRealityText ) - 1 )
			cCharacterTouched = ""
			Timer ( .055, This )
...
dw_limit_locs.Retrieve ( LeftA ( stParm.Value1, 1 ) )
...
Else
	...
	Else
		...
		i_len = LenA(trim(szdimstr_1))
		If i_len > 0 then 
			i_currow = 0 
			i_start = 1
			Do
				i_pos = PosA(szdimstr_1,',',i_start)
				If i_pos <= 0 then 
					Exit
				End if 
				i_pos -- 
				i_currow ++ 
				d_storedno[i_currow] = round(dec(MidA(szdimstr_1,i_start,i_pos)),2)
				i_pos += 2
				i_start = i_pos 
			Loop While (true)
		...
		//	retrieving the data back from the string
		ls_newdimstr=''
		i_len = LenA(trim(szdimstr_2))
		If i_len > 0 then 
			i_currow = 0 
			i_start = 1
			Do
				i_pos = PosA(szdimstr_2,',',i_start)
				If i_pos <= 0 then 
					Exit
				End if 
				i_pos -- 
				...
				if i_currow = llnonconstantdim  then 
					d_storedno[i_currow] += round(dec(MidA(szdimstr_2,i_start,i_pos)),2)
				else	
					d_storedno[i_currow] = round(dec(MidA(szdimstr_2,i_start,i_pos)),2)			
				end if 	
