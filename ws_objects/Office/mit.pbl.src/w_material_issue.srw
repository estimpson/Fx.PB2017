...
i_len = LenA(trim(ls_dim_qty_str))
If i_len > 0 then 
	i_currow = 0 
	i_start = 1
	Do
		i_pos = PosA(ls_dim_qty_str,',',i_start)
		If i_pos <= 0 then 
			Exit
		End if 
		i_pos -- 
		i_currow ++ 
		d_storedno[i_currow] = dec(MidA(ls_dim_qty_str,i_start,i_pos))
	   i_pos += 2
		i_start = i_pos 
	Loop While (true)
...
//llnonconstantdim = 1
ls_newdimstr=''
i_len = LenA(trim(ls_curdimqtystr))
If i_len > 0 then 
	i_currow = 0 
	i_start = 1
	Do
		i_pos = PosA(ls_curdimqtystr,',',i_start)
		If i_pos <= 0 then 
			Exit
		End if 
		i_pos -- 
		...
		if i_currow = llnonconstantdim  then 
			d_storedno[i_currow] += dec(MidA(ls_curdimqtystr,i_start,i_pos))
		else	
			d_storedno[i_currow] = dec(MidA(ls_curdimqtystr,i_start,i_pos))			
		end if 	
...
if li_pos > 0 then
	...
	ls_str2 = RightA(ls_orgdimstr, (li_pos - 1))
