$PBExportHeader$w_crosstab_bar_style_ancestor.srw
$PBExportComments$(mst) BAR graph ancestor user object.
forward
global type w_crosstab_bar_style_ancestor from Window
end type
type uo_date from u_date_lists within w_crosstab_bar_style_ancestor
end type
type vsb_1 from vscrollbar within w_crosstab_bar_style_ancestor
end type
type hsb_1 from hscrollbar within w_crosstab_bar_style_ancestor
end type
type ln_1 from line within w_crosstab_bar_style_ancestor
end type
type ln_2 from line within w_crosstab_bar_style_ancestor
end type
type ln_3 from line within w_crosstab_bar_style_ancestor
end type
type ln_4 from line within w_crosstab_bar_style_ancestor
end type
type ln_5 from line within w_crosstab_bar_style_ancestor
end type
type ln_6 from line within w_crosstab_bar_style_ancestor
end type
type ln_7 from line within w_crosstab_bar_style_ancestor
end type
end forward
...
global w_crosstab_bar_style_ancestor w_crosstab_bar_style_ancestor
...
public subroutine wf_draw_key_list ();/* this is a prototype function */
end subroutine
...
for ll_row = 1 to ll_total
	...
	if ld_date >= ld_start AND ld_date <= ld_grid_end_date then
		...
		lc_days			=( daysafter( ld_start, ld_date ) + &
							 (	hour( lt_time ) / 24 ) + &
							 ( minute( lt_time ) /3600 ) ) / ii_mode
...
if upperbound( is_key_list ) <= &
	...
	vsb_1.maxposition	= 	(upperbound( is_key_list ) * ii_distance_y) 
	vsb_1.position		=	( ii_end_y - ii_start_y )
end if
...
on w_crosstab_bar_style_ancestor.create
...
on w_crosstab_bar_style_ancestor.destroy
...
on moved;if this.position >= this.minPosition and this.position <= this.maxPosition then
	wf_position_y( this.minposition - this.position + ii_distance_y)
end if
end on
...
on pageup;if this.position > this.minposition then
	this.position	-= ii_distance_y
...
on lineup;if this.position > this.minposition then
	this.position	-= ii_distance_y
	wf_move_y( ii_distance_y )
end if
end on
