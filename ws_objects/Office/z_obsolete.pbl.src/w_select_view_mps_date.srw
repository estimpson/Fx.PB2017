$PBExportHeader$w_select_view_mps_date.srw
forward
global type w_select_view_mps_date from Window
end type
type cb_2 from commandbutton within w_select_view_mps_date
end type
type cb_1 from commandbutton within w_select_view_mps_date
end type
type p_prev from picture within w_select_view_mps_date
end type
type p_next from picture within w_select_view_mps_date
end type
type dw_cal from datawindow within w_select_view_mps_date
end type
type sle_date from singlelineedit within w_select_view_mps_date
end type
end forward
...
global w_select_view_mps_date w_select_view_mps_date
...
on open;Date lDate
	...
	If sle_date.text = "" then
		lDate = w_Production_grid.dDate1
		init_cal(lDate)
		sle_date.text = string(w_Production_grid.dDate1)
	Else
...
on w_select_view_mps_date.create
...
on w_select_view_mps_date.destroy
destroy(this.cb_2)
destroy(this.cb_1)
destroy(this.p_prev)
destroy(this.p_next)
...
on deactivate;Close(w_select_view_mps_date)
end on
...
on clicked;close(w_select_view_mps_date)
end on
...
on clicked;w_select_view_mps_date.visible	= FALSE
w_production_grid.dDate1 = date(nYear, nMonth, nDay)
w_production_grid.wf_rebuild_crosstab()
close(w_select_view_mps_date)
end on
