...
//	
//			: Local variables
//				lstr_generic		: To store the argument structure
//////////////////////////////////////////////////////////////////////////////////////////
//	Declarations
st_generic_structure lstr_generic

//	Argument Assignment
lstr_generic = Message.PowerObjectParm
il_serial = long(lstr_generic.value1)	// get serial #
is_nooflabels  = lstr_generic.value11

//	Identify serial (i.e whether it's a object serial or pallet serial)
select	type
into	:is_type
from	object
where	serial = :il_serial;
if is_type = 'S' then
	//	Get date from object table for that pallet serial
	//	Room for improvement in this area label per part # for a pallet
	select	max(part), sum(quantity), min(suffix), max(lot), max(shipper), max(origin), 
		max(last_date), max(custom4)
	into	:is_part, :id_quantity, :il_suffix, :is_lot, :il_shipper, :is_origin,
		:idt_objdatetime, :is_custom4
	from	object
...
event open;//	Trigger user event open
