$PBExportHeader$cw_mes_manualjob.srw
forward
...
salesOrderNo = dw_edit.object.SalesOrderNo[1]
...
message.inv_Parm.of_SetItem("SalesOrderNo", salesOrderNo)
...
salesOrderNo = object.SalesOrderNo[1]
...
choose case lower(dwo.Name)
	...
	case lower("PartCode")
		//	Show the part list for the selected machine.
		datawindowchild salesOrderNoDWC
		if	GetChild("SalesOrderNo", salesOrderNoDWC) = 1 then
			salesOrderNoDWC.SetTransObject(SQLCA)
			salesOrderNoDWC.Retrieve(data)
		end if
		partCode = data
	case lower("SalesOrderNo")
		salesOrderNo = long(data)
	case lower("QtyBuild")
		qtyBuild = Dec(data)
	case lower("DueDT")
		dueDT = datetime(data)
