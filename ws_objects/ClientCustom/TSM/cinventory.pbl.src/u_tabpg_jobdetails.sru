...
type cb_pallet from commandbutton within u_tabpg_jobdetails
end type
end forward
...
this.Control[iCurrent+6]=this.cb_pallet
end on
...
inv_Resize.of_Register(cb_pallet, 100, 0, 0, 0)
...
//	e.g.
masterDWOName[1] = String(windowHandle)+".MasterDWO.finishedpartcode"
...
choose case EventName
	case String(windowHandle)+".MasterDWO"
		Refreshing = (FinishedPartCode = isnull(eventmessage.of_GetItem(String(windowHandle)+".MasterDWO.finishedpartcode"), ""))
		FinishedPartCode = isnull(eventmessage.of_GetItem(String(windowHandle)+".MasterDWO.finishedpartcode"), "")
		Refresh()
end choose
...
return Retrieve(FinishedPartCode)
...
datawindowchild palletPackageTypesDWC
if	GetChild("PalletPackageType", palletPackageTypesDWC) = 1 then
	palletPackageTypesDWC.SetTransObject(SQLCA)
	palletPackageTypesDWC.Retrieve(object.PartCode[activerow])
end if
...
message.inv_Parm.of_SetItem("Machine", "")
...
if	message.DoubleParm = 1 then
	...
	long salesOrderNo
	long nullLong; SetNull(nullLong)
	string partCode
	...
	salesOrderNo = isnull(message.inv_Parm.of_GetItem("SalesOrderNo"), nullLong)
	...
	InventoryTrans.ScheduleManualJob(manualWorkOrderNumber, partCode, salesOrderNo, machineCodeManualJob, buildQty, dueDT)
end if
...
string machineCode, palletPackageType, palletLabelFormat
machineCode = dw_1.object.MachineCode[jobRow]
palletPackageType = dw_1.object.PalletPackageType[jobRow]
palletLabelFormat = dw_1.object.PalletLabelFormat[jobRow]

//	Open dialog to create pallets:
message.inv_Parm.of_SetItem("MachineCode", machineCode)
message.inv_Parm.of_SetItem("PalletPackageType", palletPackageType)
open(w_mes_createpallets)

//	Check return value...
if	message.DoubleParm = 1 then
	
	//	Get the number of pallets to create.
	int palletCount
	int nullInt; SetNull(nullInt)
	palletCount = isnull(message.inv_Parm.of_GetItem("PalletCount"), nullInt)
	
	//	Create pallet(s).
	long firstPalletSerial
	InventoryTrans.CreatePallets(jobID, palletPackageType, palletCount, firstPalletSerial)
	
	//	Print pallet labels.
	long palletSerial
	for	palletSerial = firstPalletSerial to firstPalletSerial + palletCount - 1

		//Print Labels
		n_cst_associative_array	printerParm
		printerParm.of_SetItem("Arg1", palletSerial)
		printerParm.of_SetItem("Report", "Label")
		printerParm.of_SetItem("Name", palletLabelFormat)
		Print(printerParm)
	next
end if
