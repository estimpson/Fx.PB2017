...
public function integer verifypackaging (long serialnumber)
...
if	not box_exist(al_serial)  then
	f_beep(3)
	...
	if	ll_row > 0  then
		if	dw_object_info.GetItemNumber(ll_row, "serial") = al_serial  then dw_object_info.DeleteRow(ll_row)
	end if
	return -1
end if
...
//	Validate parts for shipper.
//	Check box or pallet.
bScanPallet = IsNull(dw_object_info.GetItemString (1, "type") = "S", false)
if	 bScanPallet then
	//	Validate parts on pallet.
...
//	Verify staging for packed shippers.
if	sle_document.text = "Printed"  then
...
//	Validate/verify packaging against shipper requirement.
if	VerifyPackaging(al_serial) = -1 then
	return -1
end if
...
public function integer verifypackaging (long serialnumber);
//	Evaluate packaging...
string partCode, packagingCode, defaultPackagingCode
...
n_ds partPackDS
partPackDS = create n_ds
partPackDS.DataObject = "d_getpartpackage_forserial"
partPackDS.SetTransObject(SQLCA)
partPackDS.Retrieve(serialNumber)

//	Get the part and package type of box or all part/package types of boxes on pallet.
int i
for	i = 1 to partPackDS.RowCount()
	partCode = partPackDS.Object.PartCode[i]
	packagingCode = partPackDS.Object.PackagingCode[i]
	if	NullOrEmpty(packagingCode) then packagingCode = 'N/A'
	
	shippingTrans_PartPackaging.VerifyPack(iShipper, partCode, packagingCode, defaultPackagingCode, packDisabled, packEnabled, packWarn)

	//	If default, then aok.
	if	packagingCode = defaultPackagingCode then
		continue
	end if
	//	If enabled and not warn, then aok.
	if	packEnabled = 1 and packWarn <> 1 then
		continue
	end if
	//	If warn, then warn.
	if	packWarn = 1 then
		string messageText
		messageText = "You are attempting to stage a box with package type ~"" + packagingCode + "~".  The default package type for this part is ~"" + IsNull(defaultPackagingCode, "N/A") + "~".  Click OK to continue staging this serial."
		if	MessageBox(gnv_app.iapp_Object.DisplayName, messageText, Information!, OkCancel!, 2) = 1 then
			//	Verified.
			continue
		else
			//	Denied.
	...
	MessageBox(gnv_app.iapp_Object.DisplayName, "You are attempting to stage a box with package type ~"" + packagingCode + "~".  The default package type for this part is ~"" + defaultPackagingCode + "~".  This serial cannot be staged.", StopSign!)
	destroy partPackDS
	destroy shippingTrans_PartPackaging
	return -1
next
...
/*	Determine if destination list should be visible. */
n_cst_shippingtrans_options shippingTrans_Options
shippingTrans_Options = create n_cst_shippingtrans_options
long	changeDestinationValue
shippingTrans_Options.AllowChangeDestination (iShipper, changeDestinationValue)
rollback using SQLCA  ;
destroy shippingTrans_Options
dw_dest.Visible = (changeDestinationValue = 1)
dw_dest.Enabled = dw_dest.Visible
...
event dragdrop;
//Long iCurrentPallet 		//to keep the current pallet serial number
