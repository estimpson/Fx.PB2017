...
type cbx_qualityalert from checkbox within tabpage_general
end type
...
type tabpage_general from userobject within tab_part_master
cbx_qualityalert cbx_qualityalert
...
string text = "Shipping Quality Alert"
end type

event clicked;
n_cst_shippingtrans_qualityalert qualityAlertTrans
qualityAlertTrans = create n_cst_shippingtrans_qualityalert
if	qualityAlertTrans.SetPartAlert (i_s_part, checked) = SUCCESS then
	qualityAlertTrans.CommitTrans()
else
	qualityAlertTrans.RollbackTrans()
end if

destroy qualityAlertTrans
...
//	Set the custom Shipping Quality Alert flag.
int QualityAlert
...
from
	dbo.SQA_Parts sp
where
	sp.PartCode = :i_s_part
	and sp.Status = 0 using SQLCA  ;

cbx_qualityalert.checked = (QualityAlert > 0)
