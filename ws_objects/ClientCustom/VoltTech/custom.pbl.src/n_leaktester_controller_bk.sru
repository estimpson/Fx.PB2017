$PBExportHeader$n_leaktester_controller_bk.sru
forward
global type n_leaktester_controller_bk from n_cst_virtual_controller
end type
end forward

global type n_leaktester_controller_bk from n_cst_virtual_controller
end type
global n_leaktester_controller_bk n_leaktester_controller_bk
...
//if	_leakTesterTrans.CheckKanban(_leakTesterMachine, kanbanBinNumber, _operatorLeakTester, isLoadable, hasInputContents, isUnloadable, isEmptyOrSameOperator, outputWorkOrderNumber, outputWorkOrderDetailLineNo, outputJobId) <> SUCCESS then
//	return FAILURE
//end if
...
if	isHarness and isCompletableHarness then
	...
	if	not IsNull(_defectReason, "") > "" then
		...
		if	_leakTesterTrans.CompleteHarness(_operatorLeakTester,  _leakTesterMachine, serial, _kanbanNumber) <> SUCCESS then
...
on n_leaktester_controller_bk.create
call super::create
end on

on n_leaktester_controller_bk.destroy
call super::destroy
end on
