...
public function boolean checkcustomerengineeringlevel ()
end prototypes
...
If iValidRow = 0 then
	...
	if	not CheckCustomerEngineeringLeveL() then
		wf_enable_screen()
		return
	end if
...
String 	szPlants, szP
String 	szCum
String 	szDes, partCode, szMod, szPO, customerEngineeringLevel
String  ls_Customer
...
customerEngineeringLevel = dw_1.object.engineering_level[1]
...
If iOrder = -1 Then		//If this is a brand new order
	...
	If IsNull ( dw_1.object.blanket_part[1], '' ) = '' then	//If the blanket part number invalid
		...
		Return
	End If
	If	isnull(customerEngineeringLevel,'')='' then
	...
	partCode 	= dw_1.object.blanket_part[1]
	...
	WHERE	( order_header.destination	= :szDes ) AND  
		( order_header.blanket_part	= :partCode ) AND  
	...
	If (Not bClose) then
		...
		if	not CheckCustomerEngineeringLevel() then return
...
public function boolean checkcustomerengineeringlevel ();
string	destinationCustom1, customerCode

customerCode = dw_1.object.customer[1]
...
if	isnull(destinationCustom1,'N') = 'Y' then
	if	isnull(dw_1.object.engineering_level[1],'') = '' then 
		msgbox("This customer/destination needs a revision level, can't save the data otherwise", Information!)
		return false
	else
		return true
	end if 	
...
Else
	...
	If dw_1.RowCount() > 0 then
		...
		if isnull(prodEndDateTime) then prodEndDateTime = datetime('00/00/00')
		st_release.text = "Prodn. End Date: "+string(prodEndDateTime, '[shortdate]')+"                   Releases"
...
this.Control[iCurrent+13]=this.st_mini_cop
this.Control[iCurrent+14]=this.p_cop
this.Control[iCurrent+15]=this.st_history
this.Control[iCurrent+16]=this.sle_mini_cop
...
this.Control[iCurrent+21]=this.cb_note
this.Control[iCurrent+22]=this.mle_note
this.Control[iCurrent+23]=this.dw_stack
this.Control[iCurrent+24]=this.dw_2
this.Control[iCurrent+25]=this.cb_4
this.Control[iCurrent+26]=this.gb_edi
this.Control[iCurrent+27]=this.gb_2
this.Control[iCurrent+28]=this.dw_1
this.Control[iCurrent+29]=this.dw_edi_setup
this.Control[iCurrent+30]=this.p_note
this.Control[iCurrent+31]=this.gb_1
this.Control[iCurrent+32]=this.st_cop
this.Control[iCurrent+33]=this.st_release
end on
