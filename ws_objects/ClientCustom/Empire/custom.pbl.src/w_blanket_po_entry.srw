...
forward
global type w_blanket_po_entry from Window
end type
...
public subroutine wf_get_next_rowid ()
...
public function decimal wf_gettotal ()
public subroutine wf_updatedetail (string as_pricetype)
end prototypes
...
l_st_parm.document_number = il_PO

OpensheetWithParm ( w_print_preview, l_st_Parm, gnv_App.of_GetFrame(), 3, Layered! ) 
...
/* 03-23-2000 KAZ Modified to call window function wf_get_unit_of_measurement_part to populate 
						unit of measure DDDW in dw_smart_blanket_po_detail_entry.  Issue # 13193 */
/* 04-06-2000 KAZ Modified to return if PO has been closed.  Issue # 13305. */
/* 02-12-2004 MB  Modified to check non order status for part and allow to add release accordingly */
...
into	:ls_vstatus
from	vendor
where	code = :ls_vendorcode;

if isnull(ls_vstatus,'') <> 'Approved' then
...
ls_note = f_get_part_no_status ( is_part )
is_nonorder_status = Left ( ls_note, 1 )

if isnull(is_nonorder_status,'N') = 'Y' then 
...
//	Grids were not reflecting the correct values. GPH 2/27/01
	...
	dw_detail.SetItem ( ll_row, "ship_type", left(dw_header.object.ship_type[1],1))
...
l_s_note = f_get_part_no_status ( is_part )
is_nonorder_status = Left ( l_s_note, 1 )

if is_nonorder_status = 'N' then 
...
IF a_dw.object.#4[1] > 0 THEN
	...
	For l_i_Count = 1 to  a_dw.object.#4[1]
		...
		l_dw_detail.SetItem(1, "row_id",  il_next_row_id  )
		il_next_row_id  =  il_next_row_id  + 1
	...
	IF l_dw_detail.Update () > 0 THEN
		COMMIT ;
//		triggerevent ( 'ue_save_Detail' )																		-  CHG 02/23/2000 KAZ
...
If l_b_Found then
	sqlca.of_calculate_multicurrency_price ( l_dw_price_matrix.object.default_currency_unit[l_i_row], dw_header.object.currency_unit[1], &
		Truncate(l_dw_price_matrix.object.part_vendor_price_matrix_price[l_i_row] + 0.0000009, 6), l_n_Price )
Else
	sqlca.of_calculate_multicurrency_price ( '', dw_header.object.currency_unit[1], &
		Truncate(Dec(f_get_part_info(is_part, "STANDARD COST")) + 0.0000009, 6), l_n_price )
	If a_n_Qty > 0 then
		l_n_Price	= f_convert_units(is_um, "", is_part, a_n_Qty) * l_n_Price / a_n_Qty
	Else
		l_n_Price	= f_convert_units(is_um, "", is_part, 1)
	End If
End If
...
public subroutine wf_get_next_rowid ();SELECT isnull ( max (row_id), 0 ) + 1 
INTO	 :il_next_row_id
FROM	 po_Detail
WHERE  po_number = :il_po ;
...
public function decimal wf_gettotal ();dec	ld_total=0
int	li_row, li_totalrow

li_totalrow = dw_detail.rowcount()
if li_totalrow > 0 then 
	dw_detail.accepttext()
	for li_row = 1 to li_totalrow 
		ld_total = ld_total + dw_detail.object.balance[li_row]
	next 
end if 
return ld_total
end function

public subroutine wf_updatedetail (string as_pricetype);dec	ld_totalbal, ld_linebal, ld_price
int	li_row, li_totalrow
...
if li_totalrow > 0 then 
	// check for receipts
	for li_row = 1 to li_totalrow 
		if isnull(dw_detail.object.received[li_row],0) > 0 then 
			lb_process = false
		end if 
	next
	if lb_process then //process only if nothing has been received
		if as_pricetype = 'T' then 
			ld_totalbal = wf_gettotal()
			ld_price = wf_get_price(ld_totalbal)
		end if 
		for li_row = 1 to li_totalrow 
			if as_pricetype = 'L' then 
				ld_price = wf_get_price(dw_detail.object.balance[li_row])
			end if 
			dw_detail.setitem(li_row, "price", ld_price)
		next
		if dw_detail.update() > 0 then 
			commit;
		else
...
this.Control[]={this.em_over_received,&
...
event activate;IF wmainscreen.menuname <> 'm_blanket_po_entry' THEN &
   wmainscreen.changemenu(m_blanket_po_entry)

end event

event open;/* 03-23-2000 KAZ Modified to call window function wf_get_unit_of_measurement_part to populate 
						unit of measure DDDW in dw_smart_blanket_po_detail_entry.  Issue # 13193 */
...
event resize;dw_header.width	=	newwidth - 60
...
string	ls_pricecalctype='L'

ib_detail = TRUE
is_vendor = dw_header.Object.vendor_code[1]

ls_pricecalctype = trim(isnull(dw_header.object.po_header_blanket_frequency[1],'L'))
...
//		dw_detail.setitem(row, "price", wf_get_price ( ln_balance ) ) 
		if ls_pricecalctype = 'T' then 
			ld_bal = wf_gettotal()
		else
			ld_bal = ln_balance
		end if 	
		dw_detail.setitem(row, "price", wf_get_price ( ld_bal ) )
		berrormin = false
		RETURN 0
	END IF
ELSEIF dwo.name = 'balance' THEN
	in_minonorder	= wf_minon_order ( is_part, is_vendor ) 
	ln_balance = dec( data ) 	
	ln_value = ln_balance + object.received [ row ]
	IF  ln_value < in_minonorder THEN		
		MessageBox(monsys.msg_title, "You cannot order less than ~rMinimum On Order Quantity : " + string ( truncate ( in_minonorder , 0 ) ) + " .", StopSign!	)
		berrormin = true
		dw_Detail.SetText("")
...
choose case dwo.name
	...
	case "po_header_blanket_frequency"	
		wf_updatedetail(data)
