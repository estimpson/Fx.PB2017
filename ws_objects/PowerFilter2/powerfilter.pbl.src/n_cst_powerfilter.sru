...
forward
global type n_cst_powerfilter from n_base
end type
end forward
...
public function integer getfilters (ref string columnfilters[], ref integer columnnumbers[])
public function integer buildfilters (string columnfilters[], integer columnnumbers[])
public function integer buildfilters (string columnfilters[], integer columnnumbers[], boolean filternow)
end prototypes
...
event type long ue_clicked();if	not IsValid(idw_dw) then return FAILURE

string datawindowSyntax
datawindowSyntax = idw_dw.describe("DataWindow.Syntax")
if	datawindowSyntax = "?" or datawindowSyntax = "" then return FAILURE
...
IF this.checked THEN
	...
	for li_colnum = 1 to ii_maxcol
		...
		/*	Until I figure out how to do it better...
...
.					yes		no			PF_Filtered_PF6.bmp
.					yes		up			PF_FilteredUp_PF6.bmp
.					yes		down		PF_FilteredDown_PF6.bmp
.					no			no			PF_DownArrow_PF6.bmp
.					no			up			PF_SortedUp_PF6.bmp
.					no			down		PF_SortedDown_PF6.bmp
.					
.PowerFilter Datawindow Filter Service
.Copyright © 2010 James R. Reese.  All Rights Reserved.
.============================================================
...
public function integer getfilters (ref string columnfilters[], ref integer columnnumbers[]);/*===========================================================
.Object:			n_cst_powerfilter
.Method:			GetFilters (Function)
.Author:			Stimpsonee
.Date:				12/22/2011
.
.Argument(s):	 string columnFilters[]
.					 integer columnNumbers[]
.
.Returns:			 integer
.
.Description:	This function is called from an external service.
.					It returns the filter strings for all columns that have them, and the column reference numbers.
.
.PowerFilter Datawindow Filter Service
.Copyright © 2010 James R. Reese.  All Rights Reserved.
.============================================================
...
.	Date			Version	Author		Comment
.============================================================
.	12/22/2011	1.00		Stimpsonee	Initial Version
.============================================================*/
int i, filterCount = 0
string activeColumnFilters[]
int activeColumnNumbers[]

for	i = 1 to ii_MaxCol
	//combine all colfilters into master filter
	if	len(is_columnfilter[i]) > 0 then
		filterCount ++
		activeColumnFilters[filterCount] = is_ColumnFilter[i]
		activeColumnNumbers[filterCount] = i
	end if
next
columnFilters = activeColumnFilters
columnNumbers = activeColumnNumbers
return 0

end function

public function integer buildfilters (string columnfilters[], integer columnnumbers[]);
return BuildFilters(columnFilters, columnNumbers, true)

end function

public function integer buildfilters (string columnfilters[], integer columnnumbers[], boolean filternow);/*===========================================================
.Object:			n_cst_powerfilter
.Method:			buildFilters (Function)
.Author:			Stimpsonee
.Date:				12/22/2011
.
.Argument(s):	 string columnFilters[]
...
.
.Description:	This function is called from an external service.
.					It receives the new filter strings for a set of columns, and the column reference numbers.
.
.					It then updates the appropriate column filter instances in this object, and rebuilds the entire datawindow filter
.					based on all of the column filters that have been defined and passed back to this function
.
.PowerFilter Datawindow Filter Service
.Copyright © 2010 James R. Reese.  All Rights Reserved.
...
for	i = 1 to UpperBound(columnFilters)
	is_columnfilter[columnNumbers[i]] = columnFilters[i]
next
string	masterFilter = ""
...
for	i = 1 to ii_MaxCol
	...
	if	len(is_columnfilter[i]) > 0 then
		if	len(masterFilter) > 0 then
			masterFilter = masterFilter + ' AND '
		end if
		masterFilter = masterFilter + is_columnfilter[i] 
	end if
next

if	MaintainOriginalFilter and len(is_Originalfilter) > 0 then
	if	len(masterFilter) > 0 then
		masterFilter = '(' + masterFilter + ') AND (' + is_Originalfilter + ')'
	else
		masterFilter = is_Originalfilter
	end if
end if

if	len(masterFilter) > 0 AND KeepNewRows then
	masterFilter = '(' + masterFilter + ') OR ( IsRowNew() AND NOT IsRowModified() )'
END IF

if	filterNow then
	idw_dw.setredraw( FALSE)
	of_filter(masterFilter)
	of_setbuttonpics()
	idw_dw.setredraw( TRUE)
	this.event POST ue_postfilter()
end if
...
event destructor;	if isvalid(iu_powerfilter_dropdown) then
		iu_powerfilter_dropdown.dynamic of_close( ) //PB10.5 was crashing without this call to clear cancel status on cb_cancel
		iw_parent.dynamic CloseUserObject(iu_powerfilter_dropdown)
		iw_parent.dynamic CloseUserObject(iu_powerfilter_predeffilters)
		iw_parent.dynamic CloseUserObject(iu_powerfilter_monthopts)
	end if
