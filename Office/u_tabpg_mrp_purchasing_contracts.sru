HA$PBExportHeader$u_tabpg_mrp_purchasing_contracts.sru
forward
...
event dw_1::dropdown;call super::dropdown;
//	Change PO drop down to current part.
long row
row = GetRow()

if	row <= 0 or row > RowCount() then return failure

datawindowchild dwcPOList
if	GetChild("DefaultPONumber", dwcPOList) = SUCCESS then
	dwcPOList.SetTransObject(SQLCA)
	dwcPOList.Retrieve(object.PurchasedPart[row]) 
end if
