HA$PBExportHeader$u_tabpg_receivingdock_headers.sru
forward
...
n_cst_receivingdock_controller myController
myController = myParentWindow.Container.of_GetItem("Controller")

//	If no valid row, hide receiver.
if	activerow <= 0 then
	myController.HideReceiver()
	return
end if
myController.ShowReceiver(object.ReceiverNumber[activerow])
